(() => {
  ...
  var tiptap_core_esm_exports = {};
  __export(tiptap_core_esm_exports, {
  ...
  function findTextInFragment(frag, text2, from4, to) {
    for (var i = 0, pos = 0; i < frag.childCount && pos <= to; ) {
      ...
      if (pos >= from4 && childStart < to) {
        var found2 = str.lastIndexOf(text2, to - childStart - 1);
  ...
  var focus2 = (position = null, options) => ({ editor, view, tr, dispatch: dispatch2 }) => {
    options = {
      scrollIntoView: true,
      ...options
    };
  ...
  var setContent = (content2, emitUpdate = false, parseOptions = {}) => ({ tr, editor, dispatch: dispatch2 }) => {
    const { doc: doc2 } = tr;
  ...
  var toggleList = (listTypeOrName, itemTypeOrName) => ({ editor, tr, state, dispatch: dispatch2, chain, commands, can }) => {
...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  var NodeView = class {
    ...
    ignoreMutation(mutation) {
      ...
      if (this.dom.contains(mutation.target) && mutation.type === "childList" && isiOS() && this.editor.isFocused) {
        ...
        if (changedNodes.every((node4) => node4.isContentEditable)) {
          return false;
        }
      }
    ...
    deleteNode() {
      ...
      const to = from4 + this.node.nodeSize;
      this.editor.commands.deleteRange({ from: from4, to });
    }
  };
  var Tracker = class {
    ...
    map(position) {
      let deleted = false;
      const mappedPosition = this.transaction.steps.slice(this.currentStep).reduce((newPosition, step2) => {
        const mapResult3 = step2.getMap().mapResult(newPosition);
        if (mapResult3.deleted) {
          deleted = true;
        }
        return mapResult3.pos;
      }, position);
      return {
  ...
  function getDebugJSON(node4, startOffset = 0) {
    const isTopNode = node4.type === node4.type.schema.topNodeType;
    const increment = isTopNode ? 0 : 1;
    const from4 = startOffset;
    const to = from4 + node4.nodeSize;
    const marks2 = node4.marks.map((mark3) => {
      const output2 = {
        type: mark3.type.name
      };
      if (Object.keys(mark3.attrs).length) {
        output2.attrs = { ...mark3.attrs };
      }
      return output2;
    });
    const attrs = { ...node4.attrs };
    const output = {
      type: node4.type.name,
      from: from4,
      to
    };
    ...
    if (node4.content.childCount) {
      output.content = [];
      node4.forEach((child3, offset2) => {
        var _a;
        (_a = output.content) === null || _a === void 0 ? void 0 : _a.push(getDebugJSON(child3, startOffset + offset2 + increment));
      });
    }
    if (node4.text) {
      output.text = node4.text;
    }
    return output;
  }
  ...
  function posToDOMRect(view, from4, to) {
    ...
    const top = Math.min(start3.top, end2.top);
    const bottom = Math.max(start3.bottom, end2.bottom);
    const left = Math.min(start3.left, end2.left);
    const right = Math.max(start3.right, end2.right);
  ...
  // index.js
  window.Tiptap = {};
  window.Tiptap.Core = tiptap_core_esm_exports;
